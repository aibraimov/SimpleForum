...
__global__ void batcherBitonicMergesort64(float * d_out, float * d_in, int k, int j) {
...
int main(int argc, char **argv)
{
   clock_t start, stop;
    ...
    // launch the kernel
    start = clock();
    for (int stage = 2; stage <= ARRAY_SIZE; stage <<= 1)
    {
        for (int substage = stage>>1; substage > 0; substage >>= 1)
        {
             batcherBitonicMergesort64<<<1, ARRAY_SIZE>>>(d_out, d_in, stage, substage);
        }
    }
    stop = clock();
    print_elapsed(start, stop);
